...
function renderGrid(config, context) {
  const rows = packRows(config.blocks);
...
function renderPage(config, context, options) {
  ...
  if (!opts.noRefresh) {
    head += '<meta http-equiv="refresh" content="' + settings.refreshSeconds +
      "; url=" + (opts.selfPath || "/") + "?t=" + refreshCounter + '">\n';
  }

  head += "<title>" + escapeHtml(settings.title) + "</title>\n";
  head += '<link rel="stylesheet" href="/tablet.css?v=6">\n';
  ...
  let body = "<body>\n";
  body += renderGrid(config, context);
...
function render(config, options, callback) {
  ...
  collectData(config, function (data) {
    const context = {
      states: data.states,
